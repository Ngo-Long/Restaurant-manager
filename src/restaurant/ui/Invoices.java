...
import restaurent.dialog.BillJDialog;

public class Invoices extends javax.swing.JPanel {

    private Main mainInstance;

    public Invoices(Main mainInstance) {
...
//            mainInstance.displayPanels(new DiningTables(mainInstance));
    ...
    void displayUnpaidInvoices(DefaultTableModel modelPending, List<InvoicesEntity> pendingInvoices) {
        ...
        for (InvoicesEntity invoice : pendingInvoices) {
            ...
            // Get info ordered
            OrdersEntity order = new OrdersDAO().getByInvoiceId(invoiceId);
            ...
            int totalAmount = new OrderDetailsDAO().getTotalAmountByInvoiceID(invoiceId);
            String totalAmountConvert = addCommasToNumber(String.valueOf(totalAmount)) + " ₫";

            // Get name table 
            List<String> tableNames = new TablesDAO().getTableNamesByInvoiceId(invoiceId);
    ...
    void displayAllOfInvoices(DefaultTableModel modelConfirmed, List<InvoicesEntity> confirmedInvoices) {
        // Sắp xếp theo thời gian kết thúc từ gần nhất
