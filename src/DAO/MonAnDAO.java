...
public class MonAnDAO {
    ...
    public MonAnEntity getById(String id) {
        String sql = "SELECT * FROM MonAn WHERE idMonAn=?";
        List<MonAnEntity> list = getAll(sql, id);
        return list.isEmpty() ? null : list.get(0);
    }

    public void insert(MonAnEntity model) {
    ...
    public void update(MonAnEntity model) {
    ...
    private List<MonAnEntity> getAll(String sql, Object... args) {
        List<MonAnEntity> list = new ArrayList<>();

        try (ResultSet rs = JDBC.executeQuery(sql, args)) {
            while (rs.next()) {
                MonAnEntity model = readFromResultSet(rs);
                list.add(model);
            }
        } catch (SQLException ex) {
    ...
    private MonAnEntity readFromResultSet(ResultSet rs) throws SQLException {
